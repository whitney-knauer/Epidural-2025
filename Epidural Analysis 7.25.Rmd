...
#student experience little bar chart: 

df <- data.frame(
  Group = c("None", "Observed", "Performed"),
  event = c(54, 24, 21))
 
df$Group <- factor(df$Group, levels = c("None", "Observed", "Performed"))
...
df |> 
  ...
  theme_classic() +
  labs(x = NULL, 
       y = "Self Reported Prior Experience (%)")+
...
df_long <- df_long |>
  dplyr::mutate(
    Group = dplyr::recode(Group,
                   "Q10" = "Confident Supervised",
                   "Q11" = "Confident Unsupervised",
                   "Q12" = "A model enhances learning"))

df_long$Group <- factor(
  df_long$Group,
  levels = c("A model enhances learning",
             "Confident Unsupervised",
...
ggplot(df_long, aes(x = PercentSigned, y = Group, fill = Rating)) +
  ...
  geom_text(aes(label = ifelse(Percentage > 0, Percentage, "")),
            ...
            family = "Arial")  +
  scale_fill_brewer(palette = "Reds", direction = -1, 
     limits = c("Strongly_Disagree", "Disagree", "Neutral", "Agree", "Strongly_Agree"),  # << desired legend order
  labels = c("Strongly Disagree", "Disagree", "Neutral", "Agree", "Strongly Agree")
) +
  theme_classic() +
  labs(x = "Percentage of Respondents (%)", y = "", fill = "") +
...
df_long <- df %>% 
  pivot_longer(cols = -Group, names_to = "Rating", values_to = "Percentage") %>%
  mutate(
    Rating = factor(Rating, levels = c("Strongly_Disagree", "Disagree", "Neutral", "Agree", "Strongly_Agree")),
    Group = factor(Group, levels = rev(unique(Group))))

df_long <- df_long |>
  dplyr::mutate(
    Group = dplyr::recode(Group,
                   "Q10 pre" = "PRE - Confident Supervised",
                   "Q10 post" = "POST - Confident Supervised",
                   "Q11 pre" = "PRE - Confident Unsupervised", 
                   "Q11 post" = "POST - Confident Unsupervised"))

df_long$Group <- factor(
  df_long$Group,
  levels = rev(c("PRE - Confident Supervised",
             "POST - Confident Supervised",
             "PRE - Confident Unsupervised",
             "POST - Confident Unsupervised")))
...
# Separate datasets
df_agree <- df_long %>% filter(Rating != "Neutral") %>%
  mutate(PercentSigned = case_when(
    Rating %in% c("Strongly_Disagree", "Disagree") ~ -Percentage,
    TRUE ~ Percentage))

df_neutral <- df_long %>% filter(Rating == "Neutral") %>%
  mutate(PercentSigned = Percentage)

# Plot agreement/disagreement
p1 <- ggplot(df_agree, aes(x = PercentSigned, y = Group, fill = Rating)) +
  geom_bar(stat = "identity", position = "stack", color = "black") +
  geom_text(aes(
    label = ifelse(Percentage >= 1, Percentage, "")),
    position = position_stack(vjust = 0.5),
    size = 3, 
    color = "black", 
    family = "Times New Roman") +
  scale_fill_manual(
    values = rating_colors,
    labels = rev(c("Strongly Agree", "Agree", "Disagree", "Strongly Disagree")), 
    guide = guide_legend(nrow = 1)) +
  scale_x_continuous(limits = c(-100, 100), breaks = seq(-100, 100, by = 50)) +
  labs(x = "Percentage of Respondents (%)", y = "", fill = "") +
  theme_classic() +
  theme(
    legend.position = "top",
    text = element_text(size = 9, color = "black", family = "Times New Roman"))+
  theme(
  text = element_text(size = 9, family = "Times New Roman"),
  axis.text.x = element_text(color = "black", size = 9),
  axis.text.y = element_text(color = "black", size = 9))+
    scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 75)) +
theme(axis.text.y = element_text(size = 9),
  plot.margin = margin(5, 5, 5, 0))
...
# Plot neutral
p2 <- ggplot(df_neutral, aes(x = PercentSigned, y = Group, fill = Rating)) +
  geom_bar(stat = "identity", color = "black") +
    geom_text(aes(label = ifelse(PercentSigned >= 1, PercentSigned, "")),
            position = position_stack(vjust = 0.5),       
            color = "black",  
            size = 3, 
            family = "Times New Roman") +  
  scale_fill_manual(values = "#ededed") +
  scale_x_continuous(limits = c(0, 50), breaks = c(0, 50), minor_breaks = NULL) +
  theme_classic() +
  theme(
    legend.position = "top", 
...
df_long <- df_long |>
  dplyr::mutate(
    Group = dplyr::recode(Group,
                   "Q10" = "Confident Supervised",
                   "Q11" = "Confident Unsupervised",
                   "Q10a" = "Epidural model is a useful tool", 
                   "Q11a" = "Model has reinforced prior learning",
                   "Q12" = "Model has duplicated prior learning",
                   "Q13" = "Range of manual dexterity has improved", 
                   "Q14" = "Manual dexterity skills have improved",
                   "Q15" = "Received sufficient feedback from instructor",
                   "Q16" = "Model has improved confidence", 
                   "Q17" = "Model has helped build valuable skills"))

df_long$Group <- factor(
  df_long$Group,
  levels = rev(c(      "Confident Supervised",
                   "Confident Unsupervised",
                   "Epidural model is a useful tool", 
                   "Model has reinforced prior learning",
                   "Model has duplicated prior learning",
                   "Range of manual dexterity has improved", 
                   "Manual dexterity skills have improved",
                   "Received sufficient feedback from instructor",
                   "Model has improved confidence", 
                   "Model has helped build valuable skills")))
